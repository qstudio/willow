	...
	public static function format( $match = null, $args = null, $process = 'internal' ){
		...
		*/
		...
		if( self::$arguments ){
			...
			*/
			// self::$buffer_fields[ self::$willow_hash ] = self::$return; #self::$class::{ self::$method }();
			...
			render\fields::define([
			 	self::$willow_hash => self::$return
			]);

		}

		if( ! isset( self::$hash_log[self::$willow_hash] ) ) {

			self::$hash_log[self::$willow_hash] 	= [
				'hash'			=> self::$willow_hash,
				'context'		=> 'buffer' == self::$parse_context ? $willow_array[0] : self::$parse_context,
				'task'			=> 'prepare' == self::$parse_task ? $willow_array[1] : self::$parse_task,
			];
		...
		if( 
			...
			h::log( 'd:>'.$process.' -> Replacing Willow: "'.self::$willow_match.'" with Context return value: "'.self::$return.'"' );
			...
			// replace in markup ##
			parse\markup::swap( self::$willow_match, self::$return, 'willow', 'string', $process ); // '{{ '.$field.' }}'
			...
			$variable = willow\tags::wrap([ 'open' => 'var_o', 'value' => self::$willow_hash, 'close' => 'var_c' ]);

			h::log( 'd:>'.$process.' -> Replacing willow: "'.self::$willow_match.'" with Variable: "'.$variable.'"' );

			// variable::set( $variable, $position, 'variable' ); // '{{ '.$field.' }}'
			parse\markup::swap( self::$willow_match, $variable, 'willow', 'variable', $process ); // '{{ '.$field.' }}'
    ...
    public static function prepare( $args = null, $process = 'internal' ){ // type can be "buffer" or "internal"
		...
		if ( 
			...
			foreach( $matches[1] as $match => $value ) {
				...
				self::format( $match, $args, $process );
